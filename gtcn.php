...
add_option("gtcn_do_parent_check", "0");
...
register_setting('gtcn_options', 'gtcn_do_parent_check', 'intval');
...
wp_register_style('gtcn-plugin', get_stylesheet_directory_uri().'/gtcn-css.css', false, '1.1.2', 'all');		
} else {
wp_register_style('gtcn-plugin', plugins_url('gregs-threaded-comment-numbering/gtcn-css.css'), false, '1.1.2', 'all');
}
echo "\n".'<!-- Start Of Additions Generated By Greg\'s Threaded Comment Numbering Plugin 1.1.2 -->'."\n";
wp_print_styles('gtcn-plugin');
echo '<!-- End Of Additions Generated By Greg\'s Threaded Comment Numbering Plugin 1.1.2 -->'."\n";
} 
return;
} 
### Function: Thank you
...
function gtcn_parents_exist($comment_ID = 0) { 
if (get_option('gtcn_do_parent_check')) {
global $wpdb;
$thiscomment = get_comment( $comment_ID );
$parent = $thiscomment->comment_parent;
if ($parent == '') return false;
elseif (0 == $parent) return true;
else return gtcn_parents_exist($parent);
} else {return true;}
}
...
$after = '</div>';
if (get_option('gtcn_deepest_display') == '0') 
update_option('gtcn_deepest_display','2');
...
if (($gtcn_depth > $args['max_depth']) || !gtcn_parents_exist($comment_ID)) { 
