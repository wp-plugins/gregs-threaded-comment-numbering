...
$here = str_replace(basename( __FILE__),"",plugin_basename(__FILE__)); 
$name = $this->our_name;
...
function comment_counter_db_lookup ($comment_ID,$args = array(),$forcesimple=false) { 
global $wpdb;
...
function get_depth($comment_ID = 0) { 
global $wpdb;
...
function parents_exist($comment_ID = 0) { 
...
function build_output($number = array(),$placeholder=1) { 
...
global $wpdb;
if (empty($this->currentnumber_simple)) { 
...
if (($depth > $args['max_depth']) || ! $this->parents_exist($comment_ID)) { 
echo $before . $orphan_replacement . $after;
return;
}
if ($depth > $this->opt('deepest_display')) { 
...
if ( $myparent == $this->parenttrap[$depth - 1]) { 
$this->currentnumber[$depth] ++;
} 
else { 
$this->currentnumber[$depth] = 1; 
} 
} 
$this->currentnumber[$depth + 1] = ''; 
...
{
$gtcn = new gregsThreadedCommentNumbering('gtcn', '1.2.1', "Greg's Threaded Comment Numbering");
