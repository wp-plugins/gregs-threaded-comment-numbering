...
function gtcn_css() {
  if (get_option('gtcn_use_styles') == 1) { 
	if(@file_exists(TEMPLATEPATH.'/gtcn-css.css')) {
	  wp_register_style('gtcn-plugin', get_stylesheet_directory_uri().'/gtcn-css.css', false, '1.0.3', 'all');		
	} else {
	  wp_register_style('gtcn-plugin', plugins_url('gregs-threaded-comment-numbering/gtcn-css.css'), false, '1.0.3', 'all');
	}
	echo "\n".'<!-- Start Of Additions Generated By Greg\'s Threaded Comment Numbering Plugin 1.0.3 -->'."\n";
	wp_print_styles('gtcn-plugin');
	echo '<!-- End Of Additions Generated By Greg\'s Threaded Comment Numbering Plugin 1.0.3 -->'."\n";
  } 
  return;
} 
### Function: Thank you
...
function gtcn_comment_numbering( $comment_ID, $args = array(), $wrapclass = '' ) {
  ...
  if ( $args['max_depth'] > 1 && 0 == $comment->comment_parent ) { 
	if ( (!isset($gtcn_currentnumber)) || ($gtcn_currentnumber == '&#8230;') ) { 
	  $gtcn_currentnumber = gtcn_comment_counter_db_lookup ($comment_ID,$args);
	  }
	else { 
