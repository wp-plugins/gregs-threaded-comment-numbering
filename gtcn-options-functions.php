...
/*  Copyright (c) 2009 Greg Mulhauser, http://counsellingresource.com
	...
	This class assumes it sits within a file ending with "-functions.php", and it looks for ".ini" files named with the same pattern, minus the "-functions.php", and optionally located within a subdirectory that gets passed in. It has been done this way so if you only have one or two options pages and want to keep them at the same directory level as this class, you can do so, and everything will sort nicely -- but if you have a bunch of them, you can just dump them in a subdirectory.
	
	The options pages themselves are created out of ".ini" files parsed with a quick and dirty, PHP4-compatible reader that does only the bare minimum we need. The more capable PHP5 parse_ini_file might be more efficient -- I'm not sure -- but the PHP4 version of that function has problems which prevent it from doing the job for us, thus the quick and dirty function instead. If you would rather use the PHP5 version when running under PHP5, just un/comment the relevant lines in prep_settings.
	
	The rest will hopefully be self-explanatory if you have a peek at the options setup page which loads this class.
...
$this->oursettings = $this->prep_settings($dir . $subdir . $base,$instname);
if (!($this->instructions)) $this->oursettings = array_map(array($this,'do_option_replacements'),$this->oursettings);
return;
} // end constructor

function prep_settings($root = '',$instname='instructions') {
$ourpages = $this->pages;
$sub = isset ($_GET['submenu']) ? $_GET['submenu'] : '';
($sub != '') ? $filetail = "-$sub" : $filetail = '';
($sub == $instname) ? $this->instructions = true : $this->instructions = false;
$toload = WP_PLUGIN_DIR . '/' . $root . $filetail . '.ini';
...
// very quick and dirty replacement because PHP 4.4.7 fails to load arrays properly
...
function conflict_check($problemapps=array(),$name='') {
$domain = $this->domain;
...
$warningend = __('For best results, please disable the interfering plugin',$domain);
$message = <<<EOT
...
$thankspre = __("Thank you for having contributed directly to the development effort or linking to:",$domain);
$thankspost = __("I appreciate your support!",$domain);
$donation = ($donated) ? wptexturize("<p class='{$plugin_prefix}thanks'>{$thankspre} {$name}. {$thankspost}</p>") : "";
...
function do_options($full=true,$echo=true) {
